use super::{color::ColorSystem, palette::{EIGHT_BIT_PALETTE, LEGACY_WINDOWS_PALETTE, STANDARD_PALETTE}};
...
impl RGB {
  ...
  pub fn distance_to(&self, color: RGB) -> f64 {
    ...
    let g = g1 - g2;
    let b = b1 - b2;
    ((2.0 + rmean / 256.0) * r * r + 4.0 * g * g + (2.0 + (255.0 - rmean) / 256.0) * b * b).sqrt()
  }
  ...
  fn legacy_windows_ansi_codes(&self, foreground: bool) -> String {
    ...
    (if foreground { fore + number} else { back + number }).to_string()
  }
  ...
  fn true_color_ansi_codes(&self, foreground: bool) -> String {
    format!("{};2;{};{};{}", if foreground { "38" } else { "48" }, self.r, self.g, self.b)
  }
}
...
mod tests {
  ...
  fn ansi_codes() {
    ...
    println!("\x1b[{}m██\x1b[0m", color.ansi_codes(ColorSystem::Disabled, true));
    println!("\x1b[{}m██\x1b[0m", color.ansi_codes(ColorSystem::LegacyWindows, true));
    println!("\x1b[{}m██\x1b[0m", color.ansi_codes(ColorSystem::Standard, true));
    println!("\x1b[{}m██\x1b[0m", color.ansi_codes(ColorSystem::EightBit, true));
    println!("\x1b[{}m██\x1b[0m", color.ansi_codes(ColorSystem::TrueColor, true));
  }
}
